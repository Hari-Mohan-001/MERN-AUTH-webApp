...
const AdminDashboard = () => {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col bg-slate-400 max-w-3xl mx-auto">
        <div className="flex bg-slate-400 justify-between max-w-3xl">
          <h3>S.No</h3>
          <h3>Name</h3>
          <h3>Email</h3>
          <h3>Update</h3>
        ...
        </div>
        {
          filteredUsers.map((user, index) => (
            <div className="flex bg-white justify-between items-center py-2 px-4" key={index}>
          <span className="w-1/6 -mr-10">{index + 1}</span>
          <span className="w-1/6">{user.userName}</span>
          <span className="w-1/6">{user.email}</span>
          
          <button onClick={()=> handleEdit(user._id)}  className="w-1/6  bg-blue-600 rounded-lg" type="button">Edit</button>
          <button onClick={()=> handleDelete(user._id)} className="w-1/6 bg-red-600 rounded-lg " type="button">Delete</button>
